...
class FileRepository implements Repository {
  ...
  public async deleteById(id: number): Promise<FileModel | null> {
    const fileToDelete = await this.fileModel.query().findById(id);

    if (!fileToDelete) {
      return null;
    }

    const deletedFileId = await this.fileModel.query().deleteById(id);

    return deletedFileId > 0 ? fileToDelete : null;
  }
