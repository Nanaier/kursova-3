...
const isUserAdmin = createAsyncThunk<boolean, undefined, AsyncThunkConfig>(
  `${sliceName}/is-user-admin`,
  async (_, { extra }) => {
    const { authApi } = extra;

    return await authApi.isUserAdmin();
  },
);

export { getAuthenticatedUser, signIn, signOut, signUp, isUserAdmin };
